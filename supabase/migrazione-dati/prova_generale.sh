...
FASE="passo 02b"
echo; echo "== $FASE, le persone che l'anagrafe non ha"
esegui -v righe_persone_storiche_attese="$righe_persone_storiche_attese" -f "$DIR/02b_persone_fuori_anagrafe.sql" || ferma
...
"${PSQL[@]}" -d generale -At -c "
  select '  clienti ' || (select count(*) from cliente)
      ...
      || ', di cui cessati ' || (select count(*) from rapporto_lavoro where cessato)
