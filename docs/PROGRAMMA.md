...
| **AppSopralluoghi** | ~~i cinque ATECO mancanti~~ **chiusi** (`154cbcf`): non sono clienti persi, sono cinque celle di testo libero in cui qualcuno ha scritto **cosa fa l'azienda** invece di scegliere la voce, e non contengono una sola cifra. Cercandoli ne e uscita **una sesta, sbagliata al contrario** → **quanto morde il `continue` sul modulo di settore.** Sola lettura, e in tre numeri: (1) SHAMS SERVICE ha **oggi** qualcuno nominato RSPP o datore-RSPP? (2) quanti dei 262 stanno su una divisione dove `oreModuloSettore` da `null`, e di quelli quanti hanno una nomina in quei due ruoli — cioe la popolazione in cui «nessun modulo dovuto» e «divisione non risolta» **si leggono uguali**; (3) delle 262 celle, quante non cominciano col codice (ne avete gia contate due: quella e la misura della malattia, non il suo unico caso) | **separare i due significati del `null`** in `oreModuloSettore`, che e una scrittura sul vostro codice e si fa quando i tre numeri dicono quanto pesa | Perche il sesto caso non e un refuso: `37054` e il **CAP di Nogara** finito nella cella sopra il codice vero, e `risolviAteco` prende il primo gruppo di cifre. Un'impresa edile sta in archivio come **gestione reti fognarie**. Il livello non cambia — 37 e 41 sono entrambe `alto` — e **per caso, non per costruzione**. Cambia il modulo di settore, e li il difetto non e il `continue`: e che `null` vuol dire **due cose diverse** — «per questa divisione nessun modulo e dovuto» (vero per la maggior parte) e «la divisione non la so». Sul secondo significato il `continue` non salta un requisito assente: **nasconde un requisito che non sa calcolare**, e lo scadenzario non mostra un buco, mostra una riga in meno |
...
giuste. Si fa dove il `null` vuol dire due cose, cioe nel motore di AppSopralluoghi,
ed e il passo che gli e assegnato adesso.
